...
function hintToken(status: number): string {
  if (status === 401 || status === 403 || status === 404) {
    return " Set PAPERCLIP_GITHUB_TOKEN (or GITHUB_TOKEN/GH_TOKEN) in the server environment for private repository access.";
  }
  return "";
}

async function fetchText(url: string, githubToken?: string) {
  ...
  if (!response.ok) {
    throw unprocessable(
      `Failed to fetch ${url}: ${response.status}.${hintToken(response.status)}`,
    );
  }
...
async function resolveGitHubDefaultBranch(owner: string, repo: string, apiBase: string, githubToken?: string) {
...
async function resolveGitHubCommitSha(owner: string, repo: string, ref: string, apiBase: string, githubToken?: string) {
...
async function resolveGitHubPinnedRef(parsed: ReturnType<typeof parseGitHubSourceUrl>, githubToken?: string) {
  ...
  const trackingRef = parsed.explicitRef
    ? parsed.ref
    : await resolveGitHubDefaultBranch(parsed.owner, parsed.repo, apiBase, githubToken);
  const pinnedRef = await resolveGitHubCommitSha(parsed.owner, parsed.repo, trackingRef, apiBase, githubToken);
...
export function companySkillService(db: Db) {
  ...
  async function importFromSource(companyId: string, source: string, githubToken?: string): Promise<CompanySkillImportResult> {
    await ensureSkillInventoryCurrent(companyId);
